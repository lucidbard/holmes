...
var nodes = [], lastNodeId = 0, links = [], lastLinkId = 0, graph = { nodes: nodes, links: links };
// set up SVG for D3
var width = window.innerWidth - 180, height = window.innerHeight - 300, colors = d3.scale.category20();
...
var force = d3.layout.force()
    ...
    .linkDistance(150)
    .charge(-2000)
    .on('tick', tick), a = { type: "TE", x: 60, y: 3 * height / 6, fixed: true }, b = { type: "P", x: 1 * width / 3, y: 3 * height / 6, fixed: true }, c = { type: "S", x: 2 * width / 3, y: 3 * height / 6, fixed: true }, d = { type: "T", x: width - 60, y: 3 * height / 6, fixed: true };
...
$("#available_schemas").on("click", "label", function (e) {
    ...
    var control = document.getElementById(e.target.htmlFor);
...
function nodeTypePropertyTypeChange(e) {
    ...
    for (var j in propertyTypes[nodeProperty.type].typeValues) {
        $(property_line).append(generateNodeTypePropertyValue(nodeProperty.id, j, propertyTypes[nodeProperty.type].typeValues[j], null));
...
function tick(e) {
    ...
    path.attr('d', function (d) {
        var deltaX = d.target.x - d.source.x, deltaY = d.target.y - d.source.y, dist = Math.sqrt(deltaX * deltaX + deltaY * deltaY), normX = deltaX / dist, normY = deltaY / dist, sourcePadding = d.left ? cur_size + 4 : cur_size, targetPadding = d.right ? cur_size + 4 : cur_size, sourceX = d.source.x + (sourcePadding * normX), sourceY = d.source.y + (sourcePadding * normY), targetX = d.target.x - (targetPadding * normX), targetY = d.target.y - (targetPadding * normY);
            ...
            if (NodeTypes[selected_node.type].hasOwnProperty("properties") && NodeTypes[selected_node.type].properties.length > 0) {
                ...
                for (var prop in NodeTypes[selected_node.type].properties) {
                    ...
                    for (var j in propertyTypes[NodeTypes[selected_node.type].properties[prop].type].typeValues) {
                        ...
                        $(propDiv).append(generateNodeTypePropertyValue(prop, j, propertyTypes[NodeTypes[selected_node.type].properties[prop].type].typeValues[j], selected_node.properties[prop][j]));
                    }
...
function loadCurrentContent() {
    var jquery_element = $("#json_input")[0];
    if (!in_schema) {
...
function loadLocalStorage() {
    if (typeof (Storage) !== "undefined") {
        ...
        var currentSchemas = jQuery.parseJSON(window.localStorage.getItem("current_schemas"));
